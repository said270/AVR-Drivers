...
void TIMER0_void_SetTimerType(u8 Timer_Type){
	if (Timer_Type==0){
		CLEAR_BIT(TCCR0 , 6);
		CLEAR_BIT(TCCR0 , 3);
	}
...
void TIMER0_void_SetTimerPre(u16 Timer_Pre){
	if(Timer_Pre==1){
		SET_BIT(TCCR0 , 0);
		CLEAR_BIT(TCCR0 , 1);
		CLEAR_BIT(TCCR0 , 2);
...
void TIMER0_void_SetTimerEvent(u8 Timer_Event){
	if (u8 Timer_Event==10){
		CLEAR_BIT(TCCR0 , 4);
		CLEAR_BIT(TCCR0 , 5);
	}
	
	if (u8 Timer_Event==20){
		SET_BIT(TCCR0 , 4)
		CLEAR_BIT(TCCR0 , 5);
	}
	
	if (u8 Timer_Event==30){
		CLEAR_BIT(TCCR0 , 4);
		SET_BIT(TCCR0 , 5);
	}
	
	if (u8 Timer_Event==40){
		SET_BIT(TCCR0 , 4);
		SET_BIT(TCCR0 , 5);
	}
}
